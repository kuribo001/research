...
- `Realtime trains`: uu tien `ODPT Train Location / GTFS-RT`, bat dau bang `Toei` o `Tokyo`
...
- `Tokyo Metro`: chi xem la operator mo rong sau `Toei`
- `JR East`, `Tobu`, `Keikyu`: khong nam trong operator scope hien tai
...
Luu y moi nhat cho `Miyazaki / JR Kyushu`:

- Kiem tra ngay `2026-05-26` cho thay:
  - `https://api.gtfs-data.jp/v2/files?pref=45` -> `HTTP 200`, body `[]`
  - `https://api.gtfs-data.jp/v2/feeds?pref=45` -> `HTTP 200`, body `[]`
- Nghia la `GTFS Data Repository` dang hoat dong nhung khong tra ve GTFS usable cho Miyazaki
- Nguon thay the chinh thuc hien kha dung hon:
  - `JR Kyushu timetable portal`: https://www.jrkyushu-timetable.jp/
  - vi du timetable ga Miyazaki: https://www.jrkyushu-timetable.jp/cgi-bin/sp/sp-tt_dep.cgi/2890301/1000
  - `Miyazaki area timetable PDF`: https://www.jrkyushu.co.jp/byarea/kagoshima/info/__icsFiles/afieldfile/2026/03/12/nippo.kitto.pdf
  ...
  - `JR Kyushu line inventory`: https://www.jrkyushu.co.jp/company/info/data/line_km.html
  - `JR Kyushu operation info`: https://www.jrkyushu.co.jp/trains/info/
  - `JR Kyushu Train Navi`: https://www.jrkyushu.co.jp/app/lp/
