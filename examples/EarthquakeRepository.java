...
public final class EarthquakeRepository {
    ...
    public EarthquakeRepository(Connection connection) {
        this.connection = connection;
    }

    public long save(EarthquakeEventEnvelope envelope) throws SQLException {
        connection.setAutoCommit(false);
        try {
            long earthquakeEventId = upsertEventSnapshot(envelope.event());
            saveSingletons(earthquakeEventId, envelope);
            saveCollectionTables(earthquakeEventId, envelope);
            connection.commit();
            ...
            throw new SQLException("Failed to save earthquake envelope", exception);
        }
    }

    private long upsertEventSnapshot(EarthquakeEvent event) throws SQLException {
        ReportHead head = event.head();
        Long existingId = findExistingEventId(head.eventId(), event.messageCode(), head.issuedAt(), event.serial());
        if (existingId != null) {
            updateEventSnapshot(existingId, event);
            return existingId;
        }
    ...
    private void updateEventSnapshot(long eventId, EarthquakeEvent event) throws SQLException {
        ReportHead head = event.head();
        String sql = """
    ...
    private void saveSingletons(long eventId, EarthquakeEventEnvelope envelope) throws SQLException {
        upsertHypocenter(eventId, envelope.hypocenter());
        upsertComment(eventId, envelope.comment());
        upsertEewDetail(eventId, envelope.eewDetail());
        upsertSpecialInformation(eventId, envelope.specialInformation());
    }

    private void saveCollectionTables(long eventId, EarthquakeEventEnvelope envelope) throws SQLException {
        deleteByEventId("jma_earthquake_intensity_area", eventId);
        deleteByEventId("jma_earthquake_municipality_intensity", eventId);
        deleteByEventId("jma_earthquake_station_intensity", eventId);
        deleteByEventId("jma_eew_forecast_area", eventId);
        deleteByEventId("jma_earthquake_special_text_block", eventId);
        deleteByEventId("jma_earthquake_notice_item", eventId);
        deleteByEventId("jma_earthquake_count_item", eventId);

        Map<String, Long> stationIdByCode = insertStationIntensities(eventId, envelope);
        insertIntensityAreas(eventId, envelope);
        insertMunicipalityIntensities(eventId, envelope);
        insertLongPeriodMetrics(stationIdByCode, envelope);
        insertEewForecastAreas(eventId, envelope);
        insertSpecialTextBlocks(eventId, envelope);
        insertNoticeItems(eventId, envelope);
        insertCountItems(eventId, envelope);
    }

    private void upsertHypocenter(long eventId, EarthquakeHypocenter hypocenter) throws SQLException {
        deleteSingleton("jma_earthquake_hypocenter", eventId);
        if (hypocenter == null) {
            return;
        }
    ...
    private void upsertComment(long eventId, EarthquakeComment comment) throws SQLException {
        deleteSingleton("jma_earthquake_comment", eventId);
        if (comment == null) {
            return;
        }
    ...
    private void upsertEewDetail(long eventId, EewDetail detail) throws SQLException {
        deleteSingleton("jma_eew_detail", eventId);
        if (detail == null) {
            return;
        }
    ...
    private void upsertSpecialInformation(long eventId, EarthquakeSpecialInformation info) throws SQLException {
        deleteSingleton("jma_earthquake_special_information", eventId);
        if (info == null) {
            return;
        }
    ...
    private void insertIntensityAreas(long eventId, EarthquakeEventEnvelope envelope) throws SQLException {
        String sql = """
    ...
    private void insertMunicipalityIntensities(long eventId, EarthquakeEventEnvelope envelope) throws SQLException {
        String sql = """
    ...
    private Map<String, Long> insertStationIntensities(long eventId, EarthquakeEventEnvelope envelope) throws SQLException {
    ...
    private void insertLongPeriodMetrics(
    ...
    private void insertEewForecastAreas(long eventId, EarthquakeEventEnvelope envelope) throws SQLException {
        String sql = """
    ...
    private void insertSpecialTextBlocks(long eventId, EarthquakeEventEnvelope envelope) throws SQLException {
        String sql = """
    ...
    private void insertNoticeItems(long eventId, EarthquakeEventEnvelope envelope) throws SQLException {
        String sql = """
    ...
    private void insertCountItems(long eventId, EarthquakeEventEnvelope envelope) throws SQLException {
        String sql = """
    ...
    private void deleteByEventId(String tableName, long eventId) throws SQLException {
    ...
    private void deleteSingleton(String tableName, long eventId) throws SQLException {
        deleteByEventId(tableName, eventId);
    }

    private Long findStationIntensityIdForMetric(Map<String, Long> stationIdByKey, String stationCode) {
    ...
    private String stationKey(EarthquakeStationIntensity station) {
        return station.stationType() + "::" + station.stationCode();
    }

    private void setOffsetDateTime(PreparedStatement statement, int index, OffsetDateTime value) throws SQLException {
        if (value == null) {
